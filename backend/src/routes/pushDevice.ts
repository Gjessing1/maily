/**
 * The Android APK's notification poll — deliberately registered on the root context,
 * outside the JWT-gated `apiRoutes` plugin, because it authenticates differently from
 * everything else in Maily.
 *
 * Two reasons it cannot live behind the normal gate:
 *
 * - The request is made by an alarm-driven broadcast receiver in the APK, not the
 *   WebView, so it has no app session to ride on. It presents a device secret minted for
 *   it instead (push/devices.ts).
 ...
 *   background alarm can follow.
 */
import type { FastifyInstance } from 'fastify';
import { deviceForAuthHeader } from '../push/devices.js';
import { pendingForDevice } from '../push/pending.js';

export async function pushDeviceRoutes(app: FastifyInstance): Promise<void> {
  app.get('/api/push/pending', async (req, reply) => {
    ...
    return reply.header('Cache-Control', 'no-store').send({ mail: pendingForDevice(device) });
  });
}
